...
export const mockTemplatesForBuilder: ITemplate[] = [
  ...
  {
    templateId: 1,
    templateCode: "TMPL_HTTP_TRIGGER",
    templateName: "HTTP Request Trigger",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Trigger workflow when HTTP request is received",
    search: "http request trigger webhook api",
    ...
    configuration: JSON.stringify({ timeout: 30000, retries: 0 }),
    outputCode: "http_data",
  },
  {
    templateId: 2,
    templateCode: "TMPL_SCHEDULE_TRIGGER",
    ...
    search: "schedule cron time trigger automation",
    metadata: JSON.stringify({
      category: "TRIGGER",
      icon: "⏰",
      color: "#52c41a",
    ...
    rule: JSON.stringify({ cron_format: "standard", timezone_support: true }),
    configuration: JSON.stringify({ timeout: 10000, max_executions: 1000 }),
    outputCode: "schedule_data",
  },
  {
    templateId: 3,
    templateCode: "TMPL_FILE_TRIGGER",
    templateName: "File Watch Trigger",
    templateType: "trigger",
    typeCode: "trigger",
    typeName: "Trigger",
    agentCode: "AGT_FILE",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Trigger when file changes are detected",
    search: "file watch monitor filesystem trigger",
    metadata: JSON.stringify({
      category: "TRIGGER",
      icon: "📁",
      color: "#52c41a",
    ...
    configuration: JSON.stringify({ timeout: 5000, buffer_time: 1000 }),
    outputCode: "file_event",
  },
  ...
  // INTERMEDIATE Templates
  {
    templateId: 4,
    templateCode: "TMPL_VALIDATION",
    templateName: "Data Validation",
    templateType: "intermediate",
    typeCode: "intermediate",
    ...
    search: "validation data sanitize clean verify",
    metadata: JSON.stringify({
    ...
    schema: JSON.stringify({
      type: "object",
      properties: { rules: { type: "array" }, strict: { type: "boolean" } },
    }),
    body: "{{ data | validate }}",
    rule: JSON.stringify({
  ...
  },
  {
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Transform and map data between different formats",
    search: "transform map convert data format json xml",
    metadata: JSON.stringify({
      category: "INTERMEDIATE",
      icon: "🔄",
      color: "#1890ff",
    ...
    body: "{{ data | transform(mapping) }}",
    rule: JSON.stringify({
      supported_formats: ["JSON", "XML", "CSV", "YAML"],
      custom_mapping: true,
    }),
    configuration: JSON.stringify({ timeout: 15000, max_size: "10MB" }),
    outputCode: "transformed_data",
  },
  {
    templateId: 6,
    templateCode: "TMPL_API_CALL",
    templateName: "External API Call",
    templateType: "intermediate",
    typeCode: "intermediate",
    typeName: "Intermediate",
    agentCode: "AGT_HTTP",
    ...
    search: "api call http rest external service integration",
    metadata: JSON.stringify({
      category: "INTERMEDIATE",
      icon: "🌐",
      color: "#1890ff",
    ...
    body: "{{ api_call(url, method, payload) }}",
    rule: JSON.stringify({
      methods: ["GET", "POST", "PUT", "DELETE", "PATCH"],
      auth_types: ["bearer", "basic", "oauth"],
    }),
    ...
    outputCode: "api_response",
  },
  {
    templateId: 7,
    templateCode: "TMPL_DATABASE_QUERY",
    templateName: "Database Operation",
    templateType: "intermediate",
    typeCode: "intermediate",
    typeName: "Intermediate",
    agentCode: "AGT_DATABASE",
    ...
    search: "database query sql insert update select delete",
    metadata: JSON.stringify({
      category: "INTERMEDIATE",
      icon: "🗄️",
      color: "#1890ff",
    ...
    body: "{{ database.execute(query, params) }}",
    rule: JSON.stringify({
    ...
    outputCode: "query_result",
  },
  {
    templateId: 8,
    templateCode: "TMPL_CONDITION_CHECK",
    templateName: "Conditional Logic",
    templateType: "intermediate",
    typeCode: "intermediate",
    typeName: "Intermediate",
    agentCode: "AGT_LOGIC",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Execute conditional logic and routing decisions",
    search: "condition if else logic decision routing branch",
    metadata: JSON.stringify({
      category: "INTERMEDIATE",
      icon: "🔀",
      color: "#1890ff",
    ...
    body: "{{ if(condition) { true_path } else { false_path } }}",
    rule: JSON.stringify({
      operators: ["==", "!=", ">", "<", ">=", "<=", "contains"],
      logical: ["AND", "OR", "NOT"],
    }),
    configuration: JSON.stringify({ timeout: 2000, async: false }),
    outputCode: "condition_result",
  },
  // EXIT Templates
  {
    templateId: 9,
    templateCode: "TMPL_EMAIL_NOTIFICATION",
    templateName: "Send Email Notification",
    templateType: "exit",
    typeCode: "exit",
    typeName: "Exit",
    agentCode: "AGT_EMAIL",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Send email notifications and complete workflow",
    search: "email notification send complete finish",
    metadata: JSON.stringify({
      category: "EXIT",
      icon: "✉️",
      color: "#fa8c16",
    ...
    body: "{{ email.send(to, subject, body, template) }}",
    rule: JSON.stringify({ max_recipients: 100, template_support: true }),
    configuration: JSON.stringify({ timeout: 15000, provider: "sendgrid" }),
    outputCode: "email_result",
  },
  {
    templateId: 10,
    templateCode: "TMPL_LOG_RESULT",
    templateName: "Log and Complete",
    templateType: "exit",
    typeCode: "exit",
    typeName: "Exit",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Log workflow results and complete execution",
    search: "log result complete finish audit trail",
    metadata: JSON.stringify({
      category: "EXIT",
      icon: "📝",
      color: "#fa8c16",
    ...
    body: "{{ logger.log(level, message, data) }}",
    rule: JSON.stringify({
      levels: ["info", "warn", "error", "debug"],
      structured_logging: true,
    }),
    configuration: JSON.stringify({ timeout: 5000, storage: "elasticsearch" }),
    outputCode: "log_entry",
  },
  {
    templateId: 11,
    templateCode: "TMPL_WEBHOOK_RESPONSE",
    templateName: "Webhook Response",
    templateType: "exit",
    typeCode: "exit",
    typeName: "Exit",
    agentCode: "AGT_HTTP",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Send HTTP response and complete workflow",
    search: "webhook response http complete finish reply",
    metadata: JSON.stringify({
      category: "EXIT",
      icon: "📤",
      color: "#fa8c16",
    ...
    body: "{{ response.send(status_code, headers, body) }}",
    rule: JSON.stringify({
      status_codes: [200, 201, 400, 404, 500],
      content_types: ["json", "xml", "text"],
    }),
    configuration: JSON.stringify({ timeout: 10000, compression: true }),
    outputCode: "response_sent",
  },
  {
    templateId: 12,
    templateCode: "TMPL_DATA_EXPORT",
    templateName: "Export Data",
    templateType: "exit",
    typeCode: "exit",
    typeName: "Exit",
    agentCode: "AGT_FILE",
    ...
    statusCode: "ACTIVE",
    statusName: "Active",
    description: "Export processed data to file and complete workflow",
    search: "export data file csv json xml complete",
    metadata: JSON.stringify({
      category: "EXIT",
      icon: "💾",
      color: "#fa8c16",
    ...
    body: "{{ file.export(data, format, path) }}",
    rule: JSON.stringify({
      formats: ["CSV", "JSON", "XML", "Excel"],
      max_size: "100MB",
    }),
    configuration: JSON.stringify({ timeout: 30000, compression: "gzip" }),
    outputCode: "export_result",
  },
  {
    templateId: 4,
    templateCode: "TMPL_FRAUD_DETECT",
